...
def login_ui():
        ...
        if st.button("로그인"):
            ...
            if not row or not check_pw(pw, row["pw_hash"]):
                ...
                st.session_state.update(
                    ...
                    user_nick=row["nickname"] or row["name"],
                    page="dashboard"
                )
                _rerun()
    ...
    with tab2:
        ...
        if st.button("회원가입"):
            if len(name.strip()) < 1:
                st.error("이름을 입력하세요."); return
            if len(nickname.strip()) < 2:
                st.error("닉네임을 2자 이상 입력하세요."); return
            if len(pw2) < 6:
                st.error("비밀번호는 6자 이상"); return
            if get_user_by_email(email2):
                st.error("이미 가입된 이메일입니다."); return
            if get_user_by_login(nickname):
                st.error("이미 사용 중인 닉네임입니다."); return
            create_user(email2, name, nickname, pw2)
    ...
    if is_owner:
        ...
        if pick != "(선택)":
            ...
            if target and st.button("선택 멤버 제거"):
                remove_member(rid, target["id"]); st.success("제거 완료"); _rerun()
